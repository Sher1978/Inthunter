        ...
        try:
            ...
            async with engine.begin() as conn:
                try:
                    ...
                    await conn.execute(text("ALTER TABLE leads ADD COLUMN IF NOT EXISTS source_chat_id BIGINT;"))
                    await conn.execute(text("ALTER TABLE leads ADD COLUMN IF NOT EXISTS source_message_id BIGINT;"))
                    await conn.execute(text("ALTER TABLE leads ADD COLUMN IF NOT EXISTS source_chat_title VARCHAR(255);"))
                    await conn.execute(text("ALTER TABLE leads ADD COLUMN IF NOT EXISTS source_chat_username VARCHAR(255);"))
                    await conn.execute(text("ALTER TABLE leads ADD COLUMN IF NOT EXISTS source_invite_link VARCHAR(255);"))
