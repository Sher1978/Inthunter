...
async def get_platform_stats(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        active_joined_channels = (await db.execute(select(func.count(MonitoredChannel.id)).where(MonitoredChannel.status.in_(["JOINED", "ACTIVE", "PUBLIC_ACTIVE"])))).scalar() or 0
