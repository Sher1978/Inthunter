...
class TelegramIngestor:
    ...
    async def check_spambot_status(self, db_id: int):
        """Asks @SpamBot for the account's restriction status and auto-bans it if restricted."""
        node = next((n for n in self.scrapers if n.db_id == db_id), None)
        if not node or not node.app or not node.app.is_connected:
            return

        logger.info(f"🛡️ SpamBot Check: Initiating check for Userbot #{db_id}...")
        try:
            # Add wait_for to prevent Pyrogram from hanging indefinitely
            await asyncio.wait_for(node.app.send_message("spambot", "/start"), timeout=10.0)
            await asyncio.sleep(2.5) # Wait for SpamBot reply
            
            messages = []
            async for msg in node.app.get_chat_history("spambot", limit=2):
                if msg.from_user and msg.from_user.username and msg.from_user.username.lower() == "spambot":
                    if msg.text:
                        messages.append(msg.text.lower())
            
            if not messages:
                logger.warning(f"🛡️ SpamBot Check: No reply received from @SpamBot for #{db_id}")
                return
            ...
            logger.error(f"🛡️ SpamBot Check Error for #{db_id}: {e}")
