...
ПЕРВОЕ И ГЛАВНОЕ ПОЛЕ В JSON — 'reasoning': Напиши подробный, уникальный живой комментарий (1-3 предложения) на русском языке с подробным анализом ИМЕННО ЭТОГО сообщения. Объясни от себя, что конкретно автор делает в сообщении, цитируй его ключевые слова, и почему оно классифицировано как BUYER, SELLER или IGNORE. ЗАПРЕЩЕНО писать стандартные шаблонные ответы или одинаковые фразы!

{
  "reasoning": "Пользователь [TARGET_USER] спрашивает у участников чата совет по выбору визы и стоимости продления в Нячанге. Прямой запрос на подбор визовых услуг отсутствует, автор просит мнение сообщества.",
  "category": "IGNORE",
